...
runit_service 'prime-httpd' do
  action          :restart
  only_if         "test $(service --status-all | grep -cF 'prime-httpd') = 1"
end
...
%w(loki thor odin tyr).each do |layer|
  ...
  runit_service "proxyd-#{layer}" do
    action            :restart
    only_if         "test $(service --status-all | grep -cF 'proxyd-#{layer}') = 1"
  end
  ...
  (0..(node[:valhalla][:workers][:count] - 1)).step(1).each do |num|
    runit_service "workerd-#{layer}-#{num}" do
      action            :restart
      only_if         "test $(service --status-all | grep -cF 'workerd-#{layer}-#{num}') = 1"
    end
  end
end
