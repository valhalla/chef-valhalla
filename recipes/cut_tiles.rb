...
node[:valhalla][:extracts].each do |file|
  ...
  cron "apply changeset #{file}" do
    ...
    minute  '*/5'
    command <<-EOH
      cd #{node[:valhalla][:base_dir]} && #{node[:valhalla][:conf_dir]}/minutely_update.sh update #{node[:valhalla][:extracts_dir]} #{file} >> #{node[:valhalla][:log_dir]}/minutely_update_#{filename}.log 2>&1
    EOH
    only_if { node[:valhalla][:with_updates] == true }
  end
end
