...
node[:valhalla][:extracts].each do |extract|
  ...
  cron "apply changeset #{filename}" do
    user    node[:valhalla][:user][:name]
    minute  '*/5'
    command <<-EOH
      cd #{node[:valhalla][:base_dir]} && #{node[:valhalla][:conf_dir]}/minutely_update.sh update #{node[:valhalla][:extracts_dir]} #{filename} >> #{node[:valhalla][:log_dir]}/minutely_update_#{filename}.log 2>&1
    EOH
    only_if { node[:valhalla][:with_updates] == true }
  end
end
